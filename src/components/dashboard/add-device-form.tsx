...
const formSchema = z.object({
  ...
  vendor: z.enum(["cisco", "hp"], {
    required_error: "Device vendor is required.",
  }),
});
...
export function AddDeviceForm() {
  ...
  const onSubmit = async (values: AddDeviceFormValues) => {
    ...
    try {
      const result = await api.scanDevice(
        values.ipAddress,
        values.vendor,
        values.deviceType
      );
      ...
      // Check for the specific ping failure message from the backend
      if (errorMessage.includes("offline or unreachable")) {
        toast({
          title: "Device Unreachable",
          description: `Could not ping ${values.ipAddress}. Please check the IP and network connection.`,
          variant: "destructive",
        });
      } else {
        toast({
